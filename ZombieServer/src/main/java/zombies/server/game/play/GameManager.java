...
import zombies.dto.builder.GameStartedReplyBuilder;
...
//            d2=new Date();
    ...
    private void sendReply(TurnReplyBuilder turnReplyBuilder) {
        try {
            ...
            UserReply rep1 = turnReplyBuilder.build();
            ...
            UserReply rep2 = turnReplyBuilder.build();
    ...
    public void addOrder(UserInfo player, UserAction order) throws IOException {
        ...
        try {
            ...
            if (ta.getAction() == TurnAction.actionEnum.turn.ordinal()) {
                ...
                if (turn.get() % 2 == 1) {
                    UserReply userReply = ReplyBuilder.getTurnReplyBuilder().setAction(ta.getAction()).setCardId(ta.getCardId()).setPosition(ta.getPosition()).setTurnNumber(turn.get()).build();
                    lobbyManager.getRequestManager().sendReply(player2, userReply);
                } else {
                    UserReply userReply = ReplyBuilder.getTurnReplyBuilder().setAction(ta.getAction()).setCardId(ta.getCardId()).setPosition(ta.getPosition()).setTurnNumber(turn.get()).build();
                    lobbyManager.getRequestManager().sendReply(player1, userReply);
                }
            }
    ...
    private boolean checkTurnNumberCorrect(UserInfo player, TurnAction ta) throws IOException {
        if (ta.getTurnNumber() != turn.get()) {
                UserReply userReply = ReplyBuilder.getErrorReplyBuilder().setErrorCode(ErrorReply.errors.missing_turn.getId()).setErrorText(String.format("miss turn sended [%d] but current [%d]", ta.getTurnNumber(), turn.get())).build();
                lobbyManager.getRequestManager().sendReply(player, userReply);
                return false;
        }
        return true;
    }
    ...
    private boolean checkCorrectTurn(UserInfo player) throws IOException {
        ...
        if (flag) {
            UserReply userReply = ReplyBuilder.getErrorReplyBuilder().setErrorCode(ErrorReply.errors.not_you_turn.getId()).setErrorText("not you turn").build();
            lobbyManager.getRequestManager().sendReply(player, userReply);
            return false;
        }
        return !flag;
